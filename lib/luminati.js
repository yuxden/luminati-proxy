...
E.prototype.resolve_proxy = etask._fn(function*resolve_proxy(_this){
        ...
        Date.now()-timestamp<30000)
    {
        ...
        if (domain=='zproxy.lum-superproxy.io')
        {
...
E.prototype._request = etask._fn(function*(_this, req, res, head){
    if (!_this.hosts.length)
    {
    ...
    if (session)
    {
        ...
        if(_url && _url.indexOf('switchproxy')>-1)
          session.expire = now - 1;
    ...
    let bypass_proxy = ()=>{
        if (_this.bypass_proxy && _this.bypass_proxy.test(_url))
            return true;
        if (!_this.proxy_internal_bypass)
            return false;
        let domain = /[-\w]+\.(?:[-\w]+\.xn--[-\w]+|[-\w]{3,}|[-\w]+\.[-\w]{2})$/i
            .exec(url.parse(_url).hostname);
        return domain && _this.proxy_internal_bypass.includes(domain[0]);
    };
    if (bypass_proxy())
    {
